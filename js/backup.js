...
//     gBoard = buildFirstBoard(gLevel.SIZE) // empty board
//     renderFirstBoard(gBoard)
...
// function renderFirstBoard(board) { // display first empty board
//     var strHTML = ''
//     for (var i = 0; i < board.length; i++) {
//         strHTML += '<tr>'
...
//             strHTML += `<td data-i="${i}" data-j="${j}" onclick="onFirstCellClicked(this, ${i}, ${j})" class="cell ${cellClass}" >` //clicked cell
//             strHTML += `<span class="cell-content" style="visibility: hidden;">${EMPTY}</span>`
//             strHTML += '</td>'
//         }
//         strHTML += '</tr>'
//     }
...
//     for (var i = 0; i < board.length; i++) {
//         for (var j = 0; j < board[i].length; j++) {
//             if (!board[i][j].isCovered) { // recognize the first clicked cell
//                 continue
//             } else {
//                 board[i][j] = {
//                     minesAroundCount: 4,
...
// function renderUpdatedBoard(board) { //show the board visually using html
...
// function onFirstCellClicked(elCell, i, j) {
//     elCell = document.querySelector(`[data-i="${i}"][data-j="${j}"]`) //spefic cell that was clicked
//     console.log('First clicked cell:', elCell); // html info
//     uncover(elCell, i, j)
//     gGame.coveredCount--
//     console.log('EXPECTED coveredCount:', (gLevel.SIZE ** 2) - 1, 'ACTUAL:', gGame.coveredCount);
